...
@section('title', 'Edit Titik Penjemputan')

@section('content')
<div class="min-h-screen bg-[#07090e] text-slate-100 flex flex-col">
    <x-admin-nav :settings="$settings" subtitle="Perbarui Data & Tarif Antar-Jemput Lokasi" />

    <main class="flex-1 max-w-4xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-xl sm:text-2xl font-bold font-serif text-white">Edit Titik Penjemputan</h1>
        ...
        <form action="{{ route('admin.pickup-locations.update', $pickupLocation) }}" method="POST" class="glass-panel p-6 sm:p-8 rounded-3xl border border-white/10 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input
                    ...
                    id="name"
                    name="name"
                    value="{{ old('name', $pickupLocation->name) }}"
                    required
                    class="w-full p-3.5 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:border-amber-400 focus:outline-none"
                />
            </div>
            ...
            <div>
                ...
                <input
                    ...
                    id="detail"
                    name="detail"
                    value="{{ old('detail', $pickupLocation->detail) }}"
                    class="w-full p-3.5 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:border-amber-400 focus:outline-none"
                />
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            id="surcharge"
                            name="surcharge"
                            value="{{ old('surcharge', $pickupLocation->surcharge) }}"
                            min="0"
                            step="5000"
                            required
                    ...
                    <p class="text-[11px] text-slate-400 mt-1.5">Isi 0 jika bebas biaya / gratis.</p>
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        id="sort_order"
                        name="sort_order"
                        value="{{ old('sort_order', $pickupLocation->sort_order) }}"
                        min="0"
                        required
            ...
            <div class="pt-2">
                <label class="inline-flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $pickupLocation->is_active) ? 'checked' : '' }} class="w-5 h-5 rounded bg-white/10 border-white/20 text-amber-500 focus:ring-0 cursor-pointer">
